...
from .permissions import IsAdminUser # <-- (Arreglado) 
...
def transactions_list_create(request):
    ...
    if getattr(u, 'subscription', 'FREE') == 'FREE':
        ...
        if current >= FREE_LIMIT:
            return Response(
                {"detail": "FREE limit reached", "code": "limit_reached", "limit": FREE_LIMIT},
...
def transaction_detail(request, pk: int):
    ...
    if request.method == 'PUT':
        data = request.data.copy()
        ser = TransactionSerializer(tx, data=data, partial=True, context={'request': request})
        ser.is_valid(raise_exception=True)
        ser.save()
...
def export_csv(request):
    ...
    writer.writerow(['id', 'date', 'transaction_type', 'amount', 'description', 'category'])

    for t in qs:
        writer.writerow([t.id, t.date, t.transaction_type, t.amount, t.description or '', t.category or ''])
...
def admin_set_role(request, pk: int):
    ...
    if user_to_edit.id == 1 or user_to_edit.id == request.user.id:
        return Response({"detail": "No se puede cambiar el rol de este usuario"}, status=403)
