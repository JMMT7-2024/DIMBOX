...
// ✅ COMPONENTE DE INDICADOR DE PLAN - ACTUALIZADO
...
// ✅ COMPONENTE DE BARRA DE USO - ACTUALIZADO
...
// ✅ COMPONENTE DE UPGRADE PROMOTION - ACTUALIZADO
...
// ✅ TRANSACTION FORM CORREGIDO CON VERIFICACIÓN DE LÍMITES DEL AUTH CONTEXT
const TransactionForm = ({ onSaved, userPlan, currentUsage, compact = false }) => {
    ...
    // ✅ USAR getPlanLimits DEL AUTH CONTEXT
    const { getPlanLimits } = useAuth();
    ...
    // ✅ OBTENER LÍMITES ACTUALES DEL AUTH CONTEXT
...
const ProductManager = ({ isOpen, onClose, onProductSelect }) => {
    ...
    const handleSaveProduct = async () => {
        ...
        try {
            if (editingProduct) {
                await api.updateEnterpriseProduct(editingProduct.id, {
                    ...formData,
                ...
                await api.createEnterpriseProduct({
                    ...formData,
                    price: parseFloat(formData.price),
                    stock: parseInt(formData.stock) || 0
                });
    ...
    const handleDeleteProduct = async (product) => {
        if (window.confirm(`¿Eliminar producto ${product.name}?`)) {
            try {
                await api.deleteEnterpriseProduct(product.id);
                toast({
    ...
    const loadTransactions = useCallback(async (forceRefresh = false) => {
        ...
        try {
            ...
            if (error.response?.status === 404) {
                errorMessage = 'El servicio de transacciones estará disponible pronto';
            } else if (error.message?.includes('getTransactions')) {
...
// ✅ COMPONENTE PRINCIPAL DASHBOARD CORREGIDO
export default function Dashboard() {
    const auth = useAuth();
    const toast = useToast();
    const isMobile = useBreakpointValue({ base: true, md: false });

    // ✅ USO CORRECTO DEL AUTH CONTEXT ACTUALIZADO
    const {
        user: currentUser,
        getCurrentPlan,
        isPremium,
    ...
    // ✅ OBTENER INFORMACIÓN DEL PLAN CORRECTAMENTE
    const currentPlan = getCurrentPlan();
    const userIsEnterprise = isEnterprise();
    const userIsPremium = isPremium();
    const userIsFree = isFree();

    // ✅ OBTENER LÍMITES DEL PLAN DEL AUTH CONTEXT
    const planLimits = getPlanLimits();

    // ✅ VERIFICAR PERMISOS CORRECTAMENTE - LLAMANDO LAS FUNCIONES
    const canUserExport = canExport();
    const canUserUseAdvancedAnalytics = canUseAdvancedAnalytics();
    ...
    // ✅ CARGA DE USO ACTUALIZADA
    const loadUsageStats = useCallback(async () => {
        try {
            // Usar el número actual de transacciones como uso
            updateState({
                usageStats: {
                    transactions: transactions.length,
                    quickAccounts: state.quickAccountsCount
                }
            });
        } catch (error) {
    ...
    }, [transactions.length, state.quickAccountsCount, updateState]);
    ...
    const loadProfileData = useCallback(async (forceRefresh = false) => {
        ...
        try {
            ...
            // Cargar estadísticas de uso después del perfil
    ...
    }, [loadAllData, loadUsageStats, toast]);
                    ...
                    <Box>
                        ...
                        <VStack spacing={2} align="stretch" mb={3}>
                            ...
                            <UsageBar
                                ...
                                max={planLimits.maxTransactions}
                                label="Transacciones"
                            />
                                            ...
                                            <CardBody p={3}>
                                                <VStack spacing={3} align="stretch">
                                                    <HStack justify="space-between">
                                                        ...
                                                        {/* ✅ EXPORT SOLO SI ESTÁ PERMITIDO - CORREGIDO */}
                                                        {canUserExport && (
                                                            <ExcelExporter
                                    ...
                                    <HStack spacing={4} mt={2}>
                                        ...
                                        <Box minW="200px">
                                            <UsageBar
                                                ...
                                                max={planLimits.maxQuickAccounts}
